...
def add_cart(request,product_id):
    ...
    if current_user.is_authenticated:
        ...
        if request.method == 'POST':
            for item in request.POST:
                ...
                try:
                    ...
                    pass
        is_cart_item = Cart_Item.objects.filter(product=product,user=current_user).exists()

        if is_cart_item:
            cart_item = Cart_Item.objects.filter(product=product,user=current_user)
            # get the existing cart_item_variation
            # get the current cart_item_variation
            # item_id 
            ex_var_list = []
            id = []
        ...
        cart.save()
        is_cart_item = Cart_Item.objects.filter(product=product).exists()
        print(is_cart_item)
